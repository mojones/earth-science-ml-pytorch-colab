...
Open the [image-classification notebook in Google Colab](https://colab.research.google.com/github/mojones/earth-science-ml-pytorch-colab/blob/37509e8408ebf76e0af4352af72b80a74f032b00/Deep%20learning%20and%20pytorch%20-%20Colab.ipynb), then select **Runtime → Change runtime type → GPU** and run cells from the top. This copy contains only the plant-image lesson, not the earlier penguin example. Its first code cell downloads the dataset with visible `curl` progress, then verifies and extracts it. The GPU is optional; without one, the image example uses fewer epochs. Colab provides `torch` and `torchvision` already.
